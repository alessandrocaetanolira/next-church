...
export class CanteenSalesDetailService {
  ...
  async execute(id: string, input: unknown, actor: string) {
    ...
    if (action === 'reject') {
      ...
      await this.notify(result, 'cancelled');
    ...
    if (action === 'status') {
      ...
      if (!result) throw new NotFoundError('Pedido não encontrado.');
      await this.notify(result, orderStatus);
      return parseSale(result);
    }
    ...
    if (action === 'approve') {
      ...
      if (!result) throw new NotFoundError('Pedido não encontrado.');
      await this.notify(result, 'approved');
      return parseSale(result);
    }
  ...
  private async notify(sale: Awaited<ReturnType<CanteenSalesRepository['findById']>>, status: 'approved' | 'preparing' | 'ready' | 'cancelled') {
    if (!sale) return;
